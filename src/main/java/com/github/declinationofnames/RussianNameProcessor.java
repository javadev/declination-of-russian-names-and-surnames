...
public final class RussianNameProcessor {
    ...
    private static String sexM = "m";
    private static String sexF = "f";
    // именительный
    ...
    static Map<String, Map<String, List<String>>> rules =
    ...
    private static void prepareRules() {
        for (Map.Entry<String, Map<String, List<String>>> type : rules.entrySet()) {
            for (Map.Entry<String, List<String>> key : rules.get(type.getKey()).entrySet()) {
                for (int i = 0, n = rules.get(type.getKey()).get(key.getKey()).size(); i < n; i++) {
                    rules.get(type.getKey())
                            .get(key.getKey())
                            .set(i, rule(rules.get(type.getKey()).get(key.getKey()).get(i)));
                }
            }
        }
    }
    ...
    private static String rule(String rule) {
        return PATTERN.matcher(rule).matches() ? rule : "";
    }
    ...
    public static String word(String word, String sex, String wordType, String gcase) {
        ...
        // Иванов И. И.
        if (word != null
                && Pattern.compile("^[А-ЯЁ]\\.?$", Pattern.CASE_INSENSITIVE)
                        .matcher(word)
                        .matches()) {
            return word;
        }
        Map<String, List<String>> localRules = rules.get(wordType);
    ...
    private static String pick(
            String word, String sex, String gcase, List<String> rules, boolean matchWholeWord) {
        String wordLower = word == null ? "" : word.toLowerCase();
        for (String rule : rules) {
            if (ruleMatch(wordLower, sex, rule, matchWholeWord)) {
    ...
    private static boolean ruleMatch(
            String word, String sex, final String rule, boolean matchWholeWord) {
        final Matcher matcher = PATTERN.matcher(rule);

        if (matcher.matches()) {
            String ruleSex = matcher.group(1);
            String[] tests = matcher.group(2).split(",");

            if (ruleSex.equals(sexM) && sex.equals(sexF)) {
                // male by default
                return false;
            }
            if (ruleSex.equals(sexF) && !sex.equals(sexF)) {
                return false;
            }
            for (String test : tests) {
                String testWord =
                        matchWholeWord
                                ? word
                                : word.substring(Math.max(word.length() - test.length(), 0));
                if (testWord.equals(test)) {
                    return true;
                }
            }
        }
    ...
    private static String applyMod(final String word, String gcase, final String rule) {
        final Matcher matcher = PATTERN.matcher(rule);
        if (!matcher.matches()) {
            return word;
        }
